...
class PayPal_Subscriptions_Handler {
	...
	private function get_subscription_model( array $form, array $feed, array $submission_data, array $entry, $subscription_id = null ) {
		...
		$this->addon->log_debug( __METHOD__ . '(): $subscription => ' . wp_json_encode( $subscription ) );

		$response = $this->api->create_subscription( $this->prepare_for_request( $subscription ) );

		if ( is_wp_error( $response ) ) {
			$this->addon->log_error( __METHOD__ . '(): could not create subscription:' . $response->get_error_message() );
			return $response;
		}
	...
	private function maybe_create_product( array $form, array $feed, array $submission_data = array(), array $entry = array() ) {
		...
		if ( $product->get_id() ) {
			...
			// Confirm that the product exists on the current PayPal account.
			$product_details = $this->api->get_product( $product->get_id() );
			...
			$this->addon->log_debug( __METHOD__ . '(): Product does not exist on this PayPal account. Removing Product ID so we can recreate it.' );
			$product->id = null;
		}
	...
	private function maybe_create_plan( $product, $form, $feed, $submission_data = array(), $entry = array() ) {
		...
		if ( $plan->get_id() && $plan_currency === rgars( $feed, 'meta/ppcpSubscriptionPlanIDCurrency' ) ) {
			...
			// Confirm that the plan exists on the current PayPal account.
			$plan_details = $this->api->get_plan( $plan->get_id() );
			...
			$this->addon->log_debug( __METHOD__ . '(): Plan does not exist on this PayPal account.' );
		}
	...
	public function supports_form_selected_payment_methods( $form ) {
		$post_data = json_decode( html_entity_decode( htmlspecialchars( rgpost( 'gform_meta' ) ) ), true );
